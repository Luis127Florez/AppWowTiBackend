...
export const PostUser = async(req: Request , res: Response)=>{
    ...
    if(!email) return res.status(404).json({msg: "se requiere email para esta peticion"})
    ...
    try {
        ...
        if (alreadyUser) return  res.status(404).json({msg: "ya existe un usuario con ese email"});
...
export const GetUserByid = async (req: Request, res: Response)=>{
    ...
    try {
        ...
        if(!user) return res.status(404).json({msg:'no se encontro un usuario con ese id'});
        res.json(user);
    } catch (error) {
        console.log(error);
        res.status(500).json({msg: "hable con el admin"})
